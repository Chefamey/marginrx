...
export function EntryCard({ entry }: { entry: HouseEntry }) {
  return (
    <article className="panel group overflow-hidden p-5 transition hover:-translate-y-0.5 hover:border-jade/40">
      <div className="mb-5 h-1 w-20 rounded-full signal-rail transition group-hover:w-28" />
      ...
      <Link href={`/entries/${entry.id}`} className="mt-4 block text-xl font-semibold tracking-tight text-ink transition hover:text-jade">
        {entry.title}
      </Link>
